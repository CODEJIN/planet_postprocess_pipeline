...
![메인화면 전체 레이아웃](images_ko/run_all.png)
...
![Settings 패널](images_ko/settings.png)
*그림 3-1: 전역 설정 패널*
...
![Step 01 패널](images_ko/step01.png)
...
![Step 02 패널](images_ko/step02.png)
*그림 5-1: Step 02 패널 — Lucky Stacking 설정*

SER 파일에서 상위 프레임을 선별하고 스태킹하여 TIF 파일을 생성합니다. 외부 프로그램 없이 파이프라인 내부에서 자동으로 처리됩니다. Step 01이 활성화된 경우 Step 01의 SER 출력 폴더가 자동으로 연결됩니다.
...
| **AP 크기 (px)** | 64 | 32–128 (32 단위) | Alignment Point 크기입니다. 정렬 기준이 되는 소영역의 크기입니다. **64px = 기본값 (권장)**. 32px = 더 세밀한 정렬 (처리 느림), 128px = 넓은 영역 기준 (처리 빠름). |
| **반복 횟수** | 2 | 1–3 | Lucky Stacking 반복 횟수입니다. 각 반복마다 이전 스택 결과로 AP 정렬 기준을 갱신합니다. 2회 반복이 품질/속도 균형에서 권장됩니다. |

### 5.2 실시간 미리보기

우측 패널에 선택된 SER 폴더의 첫 번째 프레임과 AP(Alignment Point) 그리드가 표시됩니다. AP 크기를 변경하면 그리드가 자동으로 갱신됩니다.

---

## 6. Step 03 — 품질 평가 및 윈도우 탐지

![Step 03 패널](images_ko/step03.png)
*그림 6-1: Step 03 패널 — 품질 평가 설정*
...
![Step 04 패널](images_ko/step04.png)
*그림 7-1: Step 04 패널 — De-rotation 스태킹 설정*
...
![Step 05 패널](images_ko/step05.png)
*그림 8-1: Step 05 패널 — 마스터 이미지 웨이블릿 선명화 (좌: 설정, 우: 실시간 미리보기)*
...
### 8.2 림브 페더 폭 (edge_feather_factor)

| 파라미터 | 기본값 | 범위 | 설명 |
|----------|--------|------|------|
| **림브 페더 폭** | 2.0 | 0.0–8.0 (0.5 단위) | 디스크 가장자리(림브) 부근의 웨이블릿 감쇠 폭을 조절합니다. 레벨 L의 페더 폭 = 2^L × factor (px). **0.0** = 페더링 없음 (림브까지 풀 선명화, 링잉 발생 위험), **2.0** = 기본값(권장), **8.0** = 광폭 페더(행성 내부도 일부 감쇠됨). **Step 5 마스터 샤프닝에만 적용됩니다.** Step 8 시계열 프레임의 페더링은 Step 8 패널에서 별도로 조절합니다. |
...
![Step 06 모노 패널](images_ko/step06_mono.png)
*그림 9-1: Step 06 패널 — RGB 합성 설정과 실시간 미리보기*
...
![Step 06 컬러 패널](images_ko/step06_color.png)
*그림 9-2: Step 06 컬러 카메라 모드 — 자동 화이트밸런스 + 색수차 보정 미리보기*
...
![Step 07 패널](images_ko/step07.png)
*그림 10-1: Step 07 패널 — 웨이블릿 선명화 설정*
...
| **출력 기준 폴더** | (필수 입력) | — | 모든 단계의 결과물이 저장되는 상위 폴더입니다. Step 07 이후 단계의 입출력 폴더가 이 폴더 아래에 자동 설정됩니다. |
...
![Step 08 패널](images_ko/step08.png)
...
| 레벨 | 기본값 | 설명 |
|------|--------|------|
| L1~L6 | [200, 200, 200, 0, 0, 0] | Step 5와 동일한 구조. 시계열 프레임에만 적용됩니다. |
| **림브 페더 폭** | 2.0 | Step 5의 `edge_feather_factor`와 독립적으로 설정합니다. |
...
![Step 09 패널](images_ko/step09.png)
...
![Step 10 패널](images_ko/step10.png)
