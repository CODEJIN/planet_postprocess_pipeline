...
[Step 02] Lucky Stacking     ← SER → TIF 스태킹 (선택)
     ...
     │    │
     │    └──→ [Step 08] 시계열 RGB 합성  ← Step 2 TIF 기반 시간대별 합성 (선택)
     │              │
...
| **SER 병렬 처리** | 1 | 0–32 | 동시에 처리할 SER 파일 수입니다. **0** = 자동 (CPU 코어 수 ÷ 4). **1** = 순차 처리 (기본값). **주의: 값을 높이면 RAM 사용량이 배수로 증가**합니다. |
...
Lucky Stacking은 **AP별 독립 패치 스태킹 + 광역 Gaussian 블렌딩**을 사용합니다.

1. **전역 정렬**: 선별된 각 프레임을 림브 중심 감지로 기준 이미지에 서브픽셀 정렬합니다.
2. **AP별 독립 프레임 선별**: 각 AP에서 전역 선별 프레임의 상위 50%를 NCC(정규화 교차상관) 국소 선명도로 재랭킹합니다. AP마다 다른 프레임 서브셋이 선택됩니다 (등방성 패치 원리).
3. **서브픽셀 패치 추출**: 각 AP·프레임에 대해 NCC로 국소 시프트를 추정한 뒤 `getRectSubPix`로 서브픽셀 정확도의 패치를 추출합니다.
4. **품질 가중 스택**: 선별 패치를 품질 가중치로 누적합니다.
5. **광역 Gaussian 블렌딩**: 각 AP는 AP 크기의 2배(기본 64px → 128px) 크기의 Gaussian 페더 마스크로 캔버스에 합성됩니다. 인접 AP 블렌딩 영역이 크게 겹쳐 행성 원반 전체에 걸쳐 부드러운 전환이 보장됩니다. 웨이블릿 선명화 후에도 격자 아티팩트가 발생하지 않습니다.

이 방식은 등방성 패치 원리에 기반합니다: 대기 난류는 공간적으로 변하므로, 원반의 각 위치에서 최적 프레임 서브셋이 다릅니다. NCC 정렬은 저대비·균일 패치에서 위상 상관보다 강건합니다.
...
**Step 02 Lucky Stacking TIF 파일**을 기반으로 슬라이딩 윈도우 스태킹과 웨이블릿 선명화를 적용하여 시간대별 RGB 합성 이미지를 생성합니다. 행성 자전 시계열 분석 및 GIF 애니메이션 제작에 사용합니다.

> **선택적 단계**: 시계열 합성이 필요 없는 경우 건너뛰세요.

> **주의**: Step 08은 Step 06과 **독립적인** 합성 설정을 가집니다. Step 02 TIF를 입력으로 받아 자체 웨이블릿 선명화를 적용한 후 합성합니다. Step 07의 출력은 사용하지 않습니다.
...
### 11.A 모노 카메라 모드
![step08_mono](./images_ko/step08_mono.png)
...
### 11.B 컬러 카메라 모드
![step08_color](./images_ko/step08_color.png)
